...
tokens = ['ID','LPAREN', 'RPAREN', 'PUBLIC', 'STATIC', 'VOID', 'MAIN', 'LBRACE', 'RBRACE', 'INT', 'EQUALS', 'SEMICOLON', 'FLOAT', 'AREA', 'BASE', 'ALTURA', 'DIV', 'MULT', 'NU']
linea = 1
...
def ingreso(datos):
    ...
    for toke in token:
        ...
        estado = '->Linea {:4} Tipo {:16} Valor {:16} Posicion {:4} Categoría {:16}'.format(
            str(toke.lineno), str(toke.type), str(toke.value), str(toke.lexpos), categoria)
        lexer.append(estado)
